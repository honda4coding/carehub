...
function VisitCard({ record }: { record: any }) {
  const t = useTranslations("patient.MedicalHistoryPage");
  ...
  const tabs: { key: ExpandTab; label: string }[] = [
    ...
    ...(hasDocs ? [{ key: "documents" as ExpandTab, label: t("documents") }] : []),
  ];
  ...
  return (
    ...
    >
      ...
      >
        {/* Severity accent bar */}
        <div className={`w-[3px] shrink-0 ${styles.bar} rounded-s-2xl`} />
        ...
        <div className="flex-1 px-5 py-4">
          ...
          <h3 className="text-[15px] font-semibold text-[hsl(var(--color-text))] mb-1.5 leading-snug">
            {record.diagnosis || t("noDiagnosis")}
          </h3>
          ...
          <p className="text-[13px] text-[hsl(var(--color-text-muted))] flex items-center gap-1.5">
            ...
            <span className="font-medium text-[hsl(var(--color-text))]">
              {record.doctorId?.fullName ?? record.doctorId?.userName ?? t("doctor")}
            </span>
          ...
          {hasVitals && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {record.bloodPressure && <VitalChip icon={<LuHeartPulse />} label={t("bloodPressure")} value={record.bloodPressure} />}
              {record.pulse && <VitalChip icon={<LuActivity />} label={t("pulse")} value={`${record.pulse} bpm`} />}
              {record.temperature && <VitalChip icon={<LuThermometer />} label={t("temperature")} value={`${record.temperature}°C`} highlight={highTemp} />}
            </div>
          )}
        </div>
              ...
              <div>
                {hasVitals ? (
                  <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-2.5">
                    {[
                      { icon: <LuHeartPulse />, label: t("bloodPressure"), value: record.bloodPressure, unit: "mmHg" },
                      { icon: <LuDroplets />, label: t("sugarLevel"), value: record.sugarLevel, unit: "mg/dL" },
                      { icon: <LuActivity />, label: t("pulse"), value: record.pulse ? `${record.pulse}` : null, unit: "bpm" },
                      { icon: <LuThermometer />, label: t("temperature"), value: record.temperature ? `${record.temperature}` : null, unit: "°C", highlight: highTemp },
                      { icon: <LuRuler />, label: t("height"), value: record.height !== "-" ? record.height : null, unit: "cm" },
                      { icon: <LuWeight />, label: t("weight"), value: record.weight !== "-" ? record.weight : null, unit: "kg" },
                    ].map((v, i) =>
                      v.value ? (
                        <div
                          key={i}
                        ...
                        >
                          ...
                          <div className={`text-[18px] font-semibold leading-none ${v.highlight ? "text-[hsl(var(--color-danger))]" : "text-[hsl(var(--color-text))]"}`}>
                            {v.value}
                            <span className="text-[12px] font-normal text-[hsl(var(--color-text-muted))] ms-1">{v.unit}</span>
                          </div>
                        </div>
                      ) : null
                    )}
                  ...
                  <p className="text-[13px] text-[hsl(var(--color-text-muted))] italic text-center py-4">{t("noVitals")}</p>
                )}
              </div>
            )}
                  ...
                  <div className="space-y-2">
                    ...
                    {record.prescriptionText && (
                      <div className="mt-3 p-3 rounded-xl bg-[hsl(var(--color-bg-surface-hover))] border border-[hsl(var(--color-border))]">
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--color-text-muted))] mb-1.5">{t("rxNotes")}</p>
                        <p className="text-[13px] text-[hsl(var(--color-text))] leading-relaxed">{record.prescriptionText}</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-[13px] text-[hsl(var(--color-text-muted))] italic text-center py-4">{t("noMedicationsVisit")}</p>
                )}
              </div>
            )}
              ...
              <div>
                {record.notes ? (
                  <div className="p-4 rounded-xl bg-[hsl(var(--color-bg-surface-hover))] border border-[hsl(var(--color-border))]">
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--color-text-muted))] mb-2">{t("symptomsNotes")}</p>
                    <p className="text-[14px] text-[hsl(var(--color-text))] leading-relaxed">{record.notes}</p>
                  </div>
                ) : (
                  <p className="text-[13px] text-[hsl(var(--color-text-muted))] italic text-center py-4">{t("noNotes")}</p>
                )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                    {record.allergies?.length > 0 && (
                      <div className="p-3 rounded-xl bg-[hsl(var(--color-danger-bg))] border border-[hsl(var(--color-danger)/0.15)]">
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--color-danger)/0.8)] mb-1.5">{t("allergies")}</p>
                    ...
                    {record.chronic?.length > 0 && (
                      <div className="p-3 rounded-xl bg-[hsl(var(--color-warning-bg))] border border-[hsl(var(--color-warning)/0.15)]">
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--color-warning)/0.8)] mb-1.5">{t("chronic")}</p>
  ...
  return (
    <div className="bg-[hsl(var(--color-bg-surface))] border-b border-[hsl(var(--color-border))] px-4 md:px-6 py-3 overflow-x-auto">
      <div className="flex flex-nowrap sm:flex-wrap gap-2 items-center min-w-max sm:min-w-0">
        <span className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--color-text-muted))] shrink-0 pe-2 border-e border-[hsl(var(--color-border))]">
          {t("healthAlerts")}
        </span>
      ...
      {activeMeds.length > 0 && (
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--color-text-muted))] mb-2.5">
            {t("activeCount", { count: activeMeds.length })}
          </p>
          <div className="space-y-2">
        ...
        <div>
          ...
          <div className="relative mb-2.5">
            ...
            <input
              ...
              className="w-full ps-8 pe-3 py-2 text-[12px] font-medium rounded-xl border border-[hsl(var(--color-border))]
            ...
            />
            {search && (
              <button onClick={() => setSearch("")} className="absolute end-3 top-1/2 -translate-y-1/2 text-[hsl(var(--color-text-muted))] cursor-pointer hover:text-[hsl(var(--color-text))]">
          ...
          {pastMeds.length === 0 && search && (
            <p className="text-[12px] text-[hsl(var(--color-text-muted))] text-center py-4 italic">{t("noResults", { search })}</p>
          )}
        </div>
      ) : null}
    </div>
        ...
        <span className="text-[13px] font-semibold text-[hsl(var(--color-text))] flex items-center gap-2">
          ...
          {activeMeds.length > 0 && (
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-[hsl(var(--color-primary)/0.1)] text-[hsl(var(--color-primary))]">
              {t("activeCountBadge", { count: activeMeds.length })}
            </span>
          )}
        </span>
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        setToast({ msg: t("failedLoad"), variant: "error" });
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6">
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 border-b border-[hsl(var(--color-border))] pb-6">
            <h3 className="text-[17px] font-black text-[hsl(var(--color-text))] flex items-center gap-2 shrink-0">
              <LuHistory className="text-[hsl(var(--color-text-muted))] text-[20px]" /> {t("fullHistory")}
            </h3>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap items-end sm:items-center gap-3 w-full xl:w-auto">
              ...
              <div className="relative w-full sm:flex-1 sm:min-w-[200px] xl:w-auto xl:flex-none">
                ...
                />
                {filterText && (
                  <button onClick={() => setFilterText("")} className="absolute end-3 top-1/2 -translate-y-1/2 text-[hsl(var(--color-text-muted))] hover:text-[hsl(var(--color-text))] cursor-pointer">
          ...
          {!loading && records.length > 0 && (
            <div className="flex items-center justify-between mb-4">
              <p className="text-[12px] text-[hsl(var(--color-text-muted))] font-medium">
                {hasFilters
                  ? t("visitsFiltered", { filtered: filteredRecords.length, total: records.length })
                  : records.length === 1
                    ? t("visitTotalSingle")
                    : t("visitsTotal", { total: records.length })
                }
              </p>
              {hasFilters && (
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-[hsl(var(--color-bg-surface))] rounded-2xl border border-[hsl(var(--color-border))]">
              <p className="text-[14px] font-semibold text-[hsl(var(--color-text-muted))] mb-3">{t("noRecordsMatch")}</p>
            ...
            <div className="space-y-3">
              ...
              {!hasMoreRecords && filteredRecords.length > 4 && (
                <div className="text-center py-5 text-[12px] font-medium text-[hsl(var(--color-text-muted))]">
                  {filteredRecords.length === 1 ? t("endOfHistorySingle") : t("endOfHistory", { count: filteredRecords.length })}
                </div>
              )}
            </div>
          )}
