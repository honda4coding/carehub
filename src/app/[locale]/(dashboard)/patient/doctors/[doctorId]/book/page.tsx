...
export default function BookAppointmentPage() {
  const t = useTranslations("patient.BookAppointmentPage");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLoadError(err.message || t("failedLoadDoctor"));
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setLoadError(err.message || t("failedLoadSlots"));
      }
    })();
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setConfirmError(err.message || t("slotBooked"));
    } finally {
      setConfirming(false);
    }
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-screen bg-[hsl(var(--color-bg))]">
      ...
      <DashboardHeader
        title={step === "clinic" ? t("stepClinic")
          : step === "calendar" ? t("stepCalendar")
          : step === "confirm" ? t("stepConfirm")
          : t("stepSuccess")}
        subtitle={step === "success" ? t("youAreAllSet") : doctorName}
      ...
      <main className="flex-1 overflow-auto">
        ...
        {loading ? (
          ...
          <div className="p-6">
            <EmptyState icon={<LuCircleAlert />} title={t("errorTitle")} description={loadError} />
          </div>
          ...
          <div className="p-4 md:p-6 max-w-xl mx-auto w-full">
            {clinics.length === 0 ? (
              <EmptyState
                icon={<LuBuilding2 />}
                title={t("noClinicsTitle")}
                description={t("noClinicsDesc")}
              />
            ) : (
              <>
                <p className="text-[12px] font-semibold text-[hsl(var(--color-text-muted))] mb-5">
                  {clinics.length > 1 ? t("operatesFromMultiple", { doctorName }) : t("operatesFromSingle", { doctorName })}
                </p>
                <div className="space-y-3">
                  {clinics.map((c) => (
                    <button
                      ...
                      className="w-full text-start p-4 rounded-2xl border-2 border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-surface))] hover:border-sky-500 hover:bg-sky-50/50 transition-all group"
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {c.services && c.services.length > 0 && (
                            <div className="mt-1.5 text-[11px] font-bold text-sky-700">
                              {c.services.length === 1 ? t("serviceAvailable") : t("servicesAvailable", { count: c.services.length })}
                            </div>
                          )}
                        </div>
          ...
          <div className="flex flex-col h-full">
            ...
            {dateGroups.length === 0 ? (
              <div className="p-6">
                <EmptyState icon={<LuCircleAlert />} title={t("noOpenSlotsTitle")} description={t("noOpenSlotsDesc")} />
              </div>
            ) : (
                ...
                <div className="w-full md:w-[40vw] bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <p className="text-[14px] font-black text-[hsl(var(--color-text))]">
                      {viewMonth ? `${t(`months.${MONTHS[viewMonth.getMonth()]}`)} ${viewMonth.getFullYear()}` : ""}
                    </p>
                    <button
                      onClick={() => goToMonth(1)}
                  ...
                  <div className="grid grid-cols-7 gap-1 mb-2">
                    {WEEKDAYS.map((w) => (
                      <div key={w} className="text-center text-[10px] font-bold text-[hsl(var(--color-text-muted))]">{t(`weekdays.${w}`)}</div>
                    ))}
                  </div>
                      ...
                      return (
                        <div key={idx} className="flex items-center justify-center">
                          <button
                            ...
                            title={isAlreadyBooked ? t("alreadyBookedTooltip") : undefined}
                ...
                <div className="w-full md:w-[40vw] bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-2xl p-6 md:sticky md:top-6">
                  {!selectedGroup ? (
                    <div className="flex flex-col items-center justify-center text-center py-12">
                      ...
                      <p className="text-[13px] font-black text-[hsl(var(--color-text))]">{t("selectDay")}</p>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="bg-[hsl(var(--color-bg-soft))] rounded-xl p-3">
                          <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-[hsl(var(--color-text-muted))] mb-1">
                            <LuClock className="text-[12px]" /> {t("hours")}
                          </div>
                        ...
                        <div className="bg-[hsl(var(--color-bg-soft))] rounded-xl p-3">
                          <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-[hsl(var(--color-text-muted))] mb-1">
                            <LuCalendarDays className="text-[12px]" /> {t("available")}
                          </div>
                          <p className="text-[13px] font-black text-[hsl(var(--color-text))]">
                            {t("slotsCount", { count: selectedGroup.slots.length })}
                          </p>
                        </div>
                      </div>
                              ...
                              <div className="flex flex-col gap-2 mb-4">
                                {selectedClinic.phone && (
                                  <a href={`tel:${selectedClinic.phone}`}
                                    ...
                                    <LuPhone className="text-[15px]" /> {t("call", { phone: selectedClinic.phone })}
                                  </a>
                                )}
                                {selectedClinic.whatsapp && (
                                  <a href={`https://wa.me/${selectedClinic.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noreferrer"
                                    className="flex items-center gap-2.5 py-3 px-3.5 rounded-xl border border-sky-300 bg-sky-50 text-[12.5px] font-bold text-sky-700 hover:bg-sky-100 transition-all">
                                    <FaWhatsapp className="text-[15px]" /> {t("whatsapp", { phone: selectedClinic.whatsapp })}
                                  </a>
                                )}
                                {selectedClinic.landline && (
                                  <a href={`tel:${selectedClinic.landline}`}
                                    className="flex items-center gap-2.5 py-3 px-3.5 rounded-xl border border-sky-300 bg-sky-50 text-[12.5px] font-bold text-sky-700 hover:bg-sky-100 transition-all">
                                    <LuPhoneCall className="text-[15px]" /> {t("landline", { phone: selectedClinic.landline })}
                                  </a>
                                )}
                              </div>
                            ) : (
          ...
          <div className="p-4 md:p-6 max-w-md mx-auto">
            <div className="bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-2xl overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-sky-800 to-sky-600 px-6 py-7 text-center text-white">
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] opacity-80 mb-2">{t("yourAppointment")}</p>
              ...
              <div className="border-t-2 border-dashed border-[hsl(var(--color-border))] relative">
                <span className="absolute -start-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-[hsl(var(--color-bg))]" />
                <span className="absolute -end-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-[hsl(var(--color-bg))]" />
              </div>
              ...
              <div className="p-6 space-y-4">
                ...
                >
                  {confirming ? t("bookingAction") : t("confirmBookingAction")}
                </button>
                <button
          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
            ...
            <h2 className="text-[26px] font-black text-[hsl(var(--color-text))] mb-2">{t("allBooked")}</h2>
            <p className="text-[14px] font-medium text-[hsl(var(--color-text-muted))] mb-1">
              {t("apptWith")} <strong className="text-[hsl(var(--color-text))]">{doctorName}</strong>
            </p>
