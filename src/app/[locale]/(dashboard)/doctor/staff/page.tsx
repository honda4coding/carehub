                ...
                <div className="max-w-6xl mx-auto space-y-6">
                    {loading ? (
                        ...
                        <Card className="p-12 flex flex-col items-center justify-center text-center">
                            ...
                            <h3 className="text-xl font-bold mb-2 text-[hsl(var(--color-text))]">{t('noStaffMembersYet')}</h3>
                            <p className="text-[hsl(var(--color-text-muted))] max-w-sm">{t('addAssistantsOrSecretaries')}</p>
                        </Card>
                    ) : (
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <Card className="w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
                        <h2 className="text-xl font-black mb-4 text-[hsl(var(--color-text))]">{t('addNewStaff')}</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="space-y-3">
                                <input required placeholder={t('fullName')} className="w-full p-3 rounded-xl border border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-soft))] text-[hsl(var(--color-text))]" onChange={e => setFormData({...formData, fullName: e.target.value})} />
                                <input required type="email" placeholder={t('email')} className="w-full p-3 rounded-xl border border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-soft))] text-[hsl(var(--color-text))]" onChange={e => setFormData({...formData, email: e.target.value})} />
                                <input required type="password" placeholder={t('password')} className="w-full p-3 rounded-xl border border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-soft))] text-[hsl(var(--color-text))]" onChange={e => setFormData({...formData, password: e.target.value})} />
                                <input required placeholder={t('phoneNumber')} className="w-full p-3 rounded-xl border border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-soft))] text-[hsl(var(--color-text))]" onChange={e => setFormData({...formData, phoneNumber: e.target.value})} />
                                <input required placeholder={t('jobTitleEgSecretary')} className="w-full p-3 rounded-xl border border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-soft))] text-[hsl(var(--color-text))]" onChange={e => setFormData({...formData, jobTitle: e.target.value})} />
                            ...
                            <div className="pt-4 border-t border-[hsl(var(--color-border))]">
                                <h4 className="font-bold text-sm mb-3 text-[hsl(var(--color-text))]">{t('permissions')}</h4>
                                {['Appointments', 'Patients', 'Billing'].map(perm => (
                                    <label key={perm} className="flex items-center gap-3 mb-2 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm font-semibold text-[hsl(var(--color-text))]">{t('manage')}{perm}</span>
                                    </label>
                                ))}
                            </div>
                            ...
                            <div className="flex justify-end gap-3 pt-4">
                                ...
                                <Button type="submit" variant="primary">{t('addStaff')}</Button>
                            </div>
                        </form>
                    </Card>
                </div>
