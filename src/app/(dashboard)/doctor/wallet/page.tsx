    ...
    <div className="flex-1 flex flex-col h-screen bg-[#f8fafc] overflow-y-auto">
      ...
      <div className="p-6 md:p-8 max-w-5xl mx-auto w-full space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex flex-col items-start justify-between lg:col-span-2">
            <div className="w-full flex items-center justify-between mb-4">
              <p className="text-slate-500 text-[13px] font-bold uppercase tracking-wider">Available Balance</p>
              <div className="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center">
                <LuArrowDownToLine className="text-emerald-600 text-lg" />
              </div>
            </div>
            <p className="text-[36px] font-black text-slate-800 mb-2">
              {wallet?.availableBalance ?? 0} <span className="text-[16px] text-slate-400">EGP</span>
            </p>
            {profile?.isSuspended ? (
              <div className="w-full mt-2 bg-rose-50 border border-rose-200 rounded-xl p-3 flex items-start gap-2 text-rose-700">
              ...
              <button 
                ...
                className="w-full mt-2 bg-slate-900 text-white font-bold py-3 rounded-xl hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-[14px]"
              >
                Withdraw Funds
              </button>
            )}
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex flex-col justify-between">
            ...
            <p className="text-[30px] font-black text-slate-800">{wallet?.pendingBalance ?? 0} <span className="text-[14px] text-slate-400">EGP</span></p>
            <p className="text-[11px] text-slate-400 font-medium mt-2 leading-tight">Funds waiting for appointments to complete</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
            <div className="p-4 bg-slate-50 border-b border-slate-100">
              <p className="text-slate-500 text-[12px] font-bold uppercase tracking-wider">Lifetime Earnings Summary</p>
            </div>
            <div className="p-5 flex flex-col gap-4 flex-1 justify-between">
              <div>
                <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider mb-1">Total Paid By All Clients</p>
                <p className="text-[18px] font-black text-slate-800">
                  {wallet?.grossRevenue ?? 0} <span className="text-[11px] text-slate-400">EGP</span>
                </p>
              </div>
              <div>
                <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider mb-1">Total Platform Fees Deducted</p>
                <p className="text-[18px] font-black text-rose-600">
                  -{wallet?.feesPaid ?? 0} <span className="text-[11px] text-slate-400">EGP</span>
                </p>
              </div>
              <div className="mt-2 pt-4 border-t border-slate-100">
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1">── Current Rate ──</p>
                <p className="text-[14px] font-black text-slate-700">
                  {wallet?.myCurrentCommissionRate ?? 0}% 
                  <span className="text-slate-400 text-[12px] font-medium ml-1">({wallet?.myCurrentPlanName ? wallet.myCurrentPlanName.charAt(0).toUpperCase() + wallet.myCurrentPlanName.slice(1) : 'Free'} Plan)</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              ...
              <button 
                onClick={() => setShowPayoutModal(true)}
                className="text-[13px] font-bold text-emerald-600 hover:text-emerald-700"
              >
                Request Withdrawal
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl animate-in fade-in zoom-in-95">
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-6">
              <p className="text-[12px] text-slate-500 font-bold mb-1">FUNDS WILL BE SENT TO:</p>
              <p className="text-[14px] font-black text-slate-800">{profile?.paymentMethod} - {profile?.accountDetails}</p>
            </div>
      ...
      {showChangeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-xl animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            ...
            {setupError && <div className="mb-4 p-3 bg-rose-50 text-rose-600 rounded-xl text-[13px] font-bold">{setupError}</div>}
