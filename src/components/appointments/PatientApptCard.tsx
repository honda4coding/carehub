...
import { LuClock, LuPhone, LuMapPin, LuCreditCard, LuX } from "react-icons/lu";
      ...
      >
        ...
        <span
          className={`text-[28px] font-black leading-none tracking-tighter ${
            status === "cancelled" ? "line-through text-[hsl(var(--color-text-muted))]" : "text-[hsl(var(--color-text))]"
          }`}
        >
          {dateObj.getDate()}
        </span>
          ...
          <Link href={doctor?._id ? `/patient/doctors/${doctor._id}` : "#"} className="shrink-0 hover:opacity-80 transition-opacity">
            {docPic ? (
              <img
                src={docPic}
                alt={docName || "Doctor"}
                className="w-12 h-12 rounded-xl object-cover border border-[hsl(var(--color-border))]"
              />
            ) : (
          ...
          <div className="min-w-0 flex flex-col justify-center">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-0.5">
              <p className="text-[12px] font-bold text-[hsl(var(--color-text-muted))] flex items-center gap-1">
                <LuClock className="text-[12px] text-primary" />
                {timeLabel}
              </p>
              {docPhone && (
                <p className="text-[12px] font-bold text-[hsl(var(--color-text-muted))] flex items-center gap-1">
                  <LuPhone className="text-[11px]" />
                  {docPhone}
                </p>
              )}
              {docAddress && (
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          {status === "upcoming" && (
            <>
              ...
              >
                <LuCreditCard className="text-[14px]" />
                Pay
              </button>
              <button
