...
export default function MedicationSummaryWidget() {
    const t = useTranslations("patient.MedicationSummaryWidget");
    ...
    if (!summary || summary.activeMedicationsCount === 0) {
        return (
            <Card className="p-5 flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-[hsl(var(--color-text))] font-bold text-lg mb-1">{t("title")}</h3>
                <p className="text-sm text-[hsl(var(--color-text-muted))] mb-4">{t("noActive")}</p>
            </Card>
        );
    }
    ...
    return (
        ...
        >
            <div className="absolute top-0 end-0 w-32 h-32 bg-[hsl(var(--color-primary)/0.05)] rounded-full blur-3xl -me-10 -mt-10 opacity-60 pointer-events-none"></div>
            ...
            <div className="flex items-center justify-between mb-4 relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-bold text-[hsl(var(--color-text))] text-lg">{t("progressTitle")}</h3>
                </div>
            ...
            <div className="flex gap-4 relative z-10">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-[hsl(var(--color-text-muted))] font-medium">{t("overallProgress")}</p>
                </div>
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-[hsl(var(--color-text-muted))] font-medium">{t("adherenceRate")}</p>
                </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-[hsl(var(--color-border))] flex items-center justify-between relative z-10">
                <div className="flex items-center gap-1.5 text-sm font-bold text-[hsl(var(--color-text-muted))]">
                    <LuPill size={16} />
                    <span>{summary.activeMedicationsCount} {t("active")}</span>
                </div>
                ...
                <div className="flex gap-2">
                    {summary.currentStreak > 0 && (
                        <div className="flex items-center gap-1 text-[13px] font-bold text-[hsl(var(--color-warning))] bg-[hsl(var(--color-bg-soft))] border border-[hsl(var(--color-border))] px-2.5 py-1 rounded-md">
                            🔥 {summary.currentStreak} {t("dayStreak")}
                        </div>
                    )}
                    {summary.adherencePercentage < 80 && summary.adherencePercentage > 0 && (
                        <div className="flex items-center gap-1 text-[13px] font-bold text-[hsl(var(--color-danger))] bg-[hsl(var(--color-bg-soft))] border border-[hsl(var(--color-border))] px-2.5 py-1 rounded-md">
                            <FiAlertCircle size={14} /> {t("lowAdherence")}
                        </div>
                    )}
                </div>
            </div>
