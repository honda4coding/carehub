...
export default function EncounterHeader({
  ...
  return (
    <header className="no-print sticky top-0 z-30 bg-[hsl(var(--color-bg-surface))] border-b border-[hsl(var(--color-border))] px-3 md:px-6 py-3 flex flex-wrap lg:flex-nowrap items-center justify-between gap-4 shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-start lg:justify-end gap-3 w-full lg:w-auto shrink-0 mt-3 lg:mt-0">
        ...
        {isAssessmentMode && (
          <div className="flex items-center flex-1 sm:flex-none bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-xl p-1 shadow-sm transition-all focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/20 shrink-0 min-w-0">
            ...
            />
            <button onClick={() => onEndSession(Number(fees) || 0)} disabled={isEnding} className="bg-primary text-white text-[13px] sm:text-sm font-bold px-3 sm:px-5 py-1.5 sm:py-2 rounded-lg shadow-[0_4px_12px_hsl(var(--color-primary)/0.25)] hover:scale-[1.02] transition-transform flex items-center justify-center gap-2 disabled:opacity-50 shrink-0">
