          ...
          {activeMeds.length > 0 ? activeMeds.map((med: any, idx: number) => (
            <div key={idx} className="bg-[hsl(var(--color-success-bg))] border border-[hsl(var(--color-success))/0.2] p-2.5 rounded-lg">
              <div className="flex justify-between items-start">
                ...
                {med.isLifelong && <span className="bg-[hsl(var(--color-success))] text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">{t('lifelong')}</span>}
              </div>
              <p className="text-[10px] font-medium text-[hsl(var(--color-success))/0.7] mt-0.5">{t('prescribed')}{med.date} {t('byDr')}{med.doctorId}</p>
            </div>
          )) : <span className="text-xs text-[hsl(var(--color-text-muted))]">{t('noneActive')}</span>}
        </div>
      </div>

      <div>
        <h3 className="text-sm font-black text-[hsl(var(--color-text-muted))] flex items-center gap-2 mb-3 uppercase tracking-wider">
          <LuHistory /> {t('pastMedications')}</h3>
        <div className="space-y-2">
          {pastMeds.length > 0 ? pastMeds.slice(0, 3).map((med: any, idx: number) => (
            <div key={idx} className="bg-[hsl(var(--color-bg-soft))] border border-[hsl(var(--color-border-soft))] p-2.5 rounded-lg opacity-75 grayscale">
              ...
              <p className="text-[9px] font-medium text-[hsl(var(--color-text-muted))] mt-0.5">{t('endedPrescribed')}{med.date}</p>
            </div>
          )) : <span className="text-xs text-[hsl(var(--color-text-muted))]">{t('noPastHistory')}</span>}
          {pastMeds.length > 3 && <p className="text-[10px] text-center font-bold text-[hsl(var(--color-text-muted))]">+ {pastMeds.length - 3} {t('more')}</p>}
        </div>
      </div>
    </div>
  );
