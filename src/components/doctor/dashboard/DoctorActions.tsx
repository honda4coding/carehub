...
export const DoctorActions = ({
  ...
  const t = useTranslations("doctor.dashboard.actions");
  return (
    <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
      ...
      <div className="md:col-span-2">
        <div className="bg-gradient-doctor rounded-xl p-5 h-full flex items-center gap-4 text-white relative overflow-hidden">
          <div className="absolute -end-6 -bottom-6 w-28 h-28 bg-white/10 rounded-full blur-2xl"></div>
          <div className="absolute end-12 top-2 w-16 h-16 bg-white/5 rounded-full blur-xl"></div>
      ...
      <div className="md:col-span-2 relative">
        <div className="bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-xl p-3.5 h-full flex flex-col justify-center relative">
          <h2 className="text-[15px] font-black text-[hsl(var(--color-text))] mb-2 leading-tight">{t("startConsultation")}</h2>
          ...
          <div className="relative w-full">
            <div className="flex items-center border border-[hsl(var(--color-border))] bg-[hsl(var(--color-bg-soft))] rounded-[8px] p-0.5 focus-within:border-[hsl(var(--color-primary)/0.5)] focus-within:ring-2 focus-within:ring-[hsl(var(--color-primary)/0.1)] transition-all min-w-0">
              <LuSearch className="ms-2.5 me-2 text-[hsl(var(--color-text-muted))] text-[14px] shrink-0" />
              <input 
                type="text" 
              ...
              <Button 
                ...
                className="!text-[12px] !px-4 !py-1.5 !h-auto !rounded-[6px] ms-0.5 shrink-0"
              >
            ...
            {showSearchResults && (
              <div className="absolute top-full end-0 mt-2 w-full md:w-[400px] bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                <div className="p-2 border-b border-[hsl(var(--color-border-soft))] bg-[hsl(var(--color-bg-soft))] flex justify-between items-center">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[hsl(var(--color-text-muted))]">{t("searchResults")}</span>
                  <button onClick={() => setShowSearchResults(false)} className="text-[10px] text-[hsl(var(--color-danger))] font-bold hover:underline cursor-pointer">{t("close")}</button>
                </div>
      ...
      <div className="md:col-span-1">
        <div className="bg-[hsl(var(--color-bg-surface))] border border-[hsl(var(--color-border))] rounded-xl p-3.5 h-full flex flex-row md:flex-col items-center md:items-stretch justify-between relative">
          <div className="flex items-center gap-3 mb-0 md:mb-2">
            ...
            <div>
              <h2 className="text-[hsl(var(--color-text))] text-[16px] font-black leading-tight">{t("walkIn")}</h2>
              <p className="text-[hsl(var(--color-text-muted))] text-[12px] font-semibold">{t("offlinePatients")}</p>
            </div>
          </div>
          <Button 
            size="sm"
